...
contract DEX {
    ...
    function price(
        ...
        return
            (yReserves * xInputWithFee) / ((xReserves * 1000) + xInputWithFee);
    }
    ...
    function ethToToken() public payable returns (uint256) {
        ...
        uint256 ethReserve = getEthReserve() - eth;
        uint256 tokenReserve = getTokenReserve();
        uint256 tokenOutput = price(eth, ethReserve, tokenReserve);
        require(eth > 0, "cannot swap 0 ETH");
        require(
            token.transfer(msg.sender, tokenOutput),
            "ethToToken(): reverted swap."
        );
        emit EthToTokenSwap(msg.sender, "Eth to Balloons", eth, tokenOutput);
        return tokenOutput;
    }
    ...
    function deposit() public payable returns (uint256) {
        ...
        uint256 ethReserve = getEthReserve() - eth;
        uint256 tokenReserve = getTokenReserve();
        uint256 tokensDeposited = ((eth * tokenReserve) / ethReserve) + 1;
        require(
            token.transferFrom(msg.sender, address(this), tokensDeposited),
            "deposit: Failed"
        );
        uint256 lpAmount = (msg.value * totalLiquidity) / ethReserve;
        liquidity[msg.sender] = liquidity[msg.sender] + lpAmount;
        totalLiquidity = totalLiquidity + lpAmount;
        ...
        returns (uint256 eth_amount, uint256 token_amount)
    {
        uint256 ethReserve = getEthReserve();
        uint256 tokenReserve = getTokenReserve();
        uint256 ethAmount = (ethReserve * amount) / totalLiquidity;
